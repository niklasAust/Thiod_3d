...
namespace Pinwheel.Griffin
{
    public static partial class GEditorCommon
    {
        ...
        private class FoldoutStyle
        {
            private static GUIStyle root;
            public static GUIStyle Root
            {
                get
                {
                    if (root == null)
                    {
                        root = new GUIStyle();
                        root.margin = new RectOffset(0, 0, 6, 6);
                    }
                    return root;
                }
            }
            ...
            public static GUIStyle HeaderBox
            {
                get
                {
                    if (headerBox == null)
                    {
                        headerBox = new GUIStyle();
                    }
                    return headerBox;
                }
            }

            private static GUIStyle bodyBox;
            public static GUIStyle BodyBox
            {
                get
                {
                    if (bodyBox == null)
                    {
                        bodyBox = new GUIStyle(EditorStyles.inspectorDefaultMargins);
                        bodyBox.padding = new RectOffset(20, 4, 4, 5);
                    }
                    return bodyBox;
                }
            }
        ...
        public static bool Foldout(GUIContent label, bool defaultExpanded, string id, Action innerGUI, GenericMenu context = null, string headerWarning = null)
        {
            ...
            EditorGUILayout.BeginVertical(FoldoutStyle.Root);
            {
                Rect headerRect = EditorGUILayout.BeginHorizontal(FoldoutStyle.HeaderBox);
                {
                    DrawHeaderBox(headerRect);

                    Rect caretRect = EditorGUILayout.GetControlRect(GUILayout.Width(14));
                    GUI.Label(caretRect, expanded ? caretDown : caretRight, EditorStyles.miniLabel);
                    Rect labelRect = EditorGUILayout.GetControlRect();

                    Vector2 lastIconSize = EditorGUIUtility.GetIconSize();
                    Vector2 headerIconSize = new Vector2(12, 12);
                    EditorGUIUtility.SetIconSize(headerIconSize);
                ...
                if (expanded)
                {
                    Rect bodyRect = EditorGUILayout.BeginVertical(FoldoutStyle.BodyBox);
                    {
                        DrawBodyBox(bodyRect, false);
                        if (innerGUI != null)
                            innerGUI.Invoke();
                    }
                    EditorGUILayout.EndVertical();
        ...
        public static void DrawAffLinks(string text, string url, string utmCampaign = "", string utmSource = "", string utmMedium = "", GUILocation uiLocation = GUILocation.InspectorUtils)
        {
            DrawAffLinks(text, new string[] { url }, utmCampaign, utmSource, utmMedium);
        }

        public static void DrawAffLinks(string text, string[] urls, string utmCampaign = "", string utmSource = "", string utmMedium = "", GUILocation uiLocation = GUILocation.InspectorUtils)
        {
            if (GEditorSettings.Instance.general.enableAffiliateLinks)
            {
                ...
                if (GUI.Button(r, content, AffLinkGUI.LinkButtonStyle))
                {
                    foreach (string s in urls)
                    {
                        ...
                        {
                            GNetUtils.TrackClick(utmMedium, uiLocation);

                            string url = GNetUtils.ModURL(s, utmCampaign, utmSource, utmMedium);
                            Application.OpenURL(url);
                        }
                    }
                }
        ...
        internal static void DrawLivePreviewToggle(ref bool livePreviewEnabled)
        {
            const string label = "Enable Live Preview";
            if (GRuntimeSettings.isHighlandEditionInstalled)
            {
                livePreviewEnabled = EditorGUILayout.Toggle(label, livePreviewEnabled);
            }
            else
            {
                const string tooltip = "Additional gears in Highland or Summit edition";
                GUIContent labelContent = EditorGUIUtility.TrTextContent(label, tooltip, GEditorSkin.Instance.GetTexture("IconWhite"));
                livePreviewEnabled = false;
                GUI.enabled = false;
                EditorGUILayout.Toggle(labelContent, false);
                GUI.enabled = true;
            }
        }
        ...
        public static bool SelectionCard(Texture2D image, string title, string description, bool selected, bool allowImageExpand = false)
        {
        ...
        public static bool ClickableCard(Texture2D image, string title, string description, string tooltip = "")
        {
            Vector2 cardSize = new Vector2(160, 200);
            Rect r = EditorGUILayout.BeginVertical(CardGUI.CardStyle, GUILayout.Width(cardSize.x), GUILayout.Height(cardSize.y));
            ...
            {
                GUIContent tmpTooltip = EditorGUIUtility.TrTextContent(string.Empty, tooltip);
                GUI.Box(r, tmpTooltip, GUIStyle.none);
            }
            ...
            {
                CardGUI.mouseTriggerRect = r;
                CardGUI.isMouseTriggered = true;
            }
            ...
            if (Event.current.type == EventType.MouseUp &&
                Event.current.button == 0)
            {
                CardGUI.isMouseTriggered = false;
            }
            ...
            if (GUI.enabled && CardGUI.isMouseTriggered && CardGUI.mouseTriggerRect == r)
            {
                EditorGUI.DrawRect(r, CardGUI.clickedColor);
            }
            else if (GUI.enabled && !CardGUI.isMouseTriggered && r.Contains(Event.current.mousePosition))
            {
                EditorGUI.DrawRect(r, CardGUI.hoverColor);
            }

            //GEditorCommon.DrawOutlineBox(r, GUI.enabled ? Color.cyan : Color.red);
            if (!GUI.enabled)
            {
                EditorGUI.DrawRect(r, CardGUI.disabledColor);
            }
        ...
        public static bool SnapObjectToSceneViewLook(GameObject g)
        {
            SceneView sv = SceneView.lastActiveSceneView;
            if (sv == null)
                return false;

            Ray ray = sv.camera.ViewportPointToRay(new Vector3(0.5f, 0.5f));
            RaycastHit hit;
            if (UnityEngine.Physics.Raycast(ray, out hit, 100))
            {
