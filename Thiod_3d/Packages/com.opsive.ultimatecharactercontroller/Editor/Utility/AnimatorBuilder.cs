...
namespace Opsive.UltimateCharacterController.Editor.Utility
{
    ...
    public static class AnimatorBuilder
    {
        ...
        public static string GenerateAnimatorCode(AnimatorController[] animatorControllers, AnimatorController[] firstPersonAnimatorControllers, string parameterName, float parameterValue, object parentObject, string baseDirectory)
        {
            ...
            if (animatorControllers != null) {
                for (int i = 0; i < animatorControllers.Length; ++i) {
                    ...
                    generatedStateMachineCode = GenerateAnimatorCode(animatorControllers[i], "animatorControllers", parameterName, parameterValue, generatedCode) || generatedStateMachineCode;
            ...
            if (firstPersonAnimatorControllers != null) {
                for (int i = 0; i < firstPersonAnimatorControllers.Length; ++i) {
                    ...
                    generatedStateMachineCode = GenerateAnimatorCode(firstPersonAnimatorControllers[i], "firstPersonAnimatorControllers", parameterName, parameterValue, generatedCode) || generatedStateMachineCode;
            ...
            if (generatedStateMachineCode) {
                ...
                var path = EditorUtility.SaveFilePanel("Save Item", baseDirectory, parentName + "Drawer.cs", "cs");
                if (path.Length != 0 && Application.dataPath.Length < path.Length) {
                    ...
                    if (File.Exists(path)) {
                        ...
                        fileString.AppendLine("{");
                        fileString.AppendLine("\tusing Opsive.Shared.Editor.UIElements.Controls;");
                        fileString.AppendLine("\tusing Opsive.UltimateCharacterController.Editor.Controls.Types.AbilityDrawers;");
                        ...
                        fileString.AppendLine("\t/// </summary>");
                        fileString.AppendLine("\t[ControlType(typeof(" + parentObject.GetType().FullName + "))]");
                        fileString.AppendLine("\tpublic class " + fileName + " : AbilityDrawer");
                        fileString.AppendLine("\t{");
                    }
                    ...
                    fileString.AppendLine("\t\t/// </summary>");
                    fileString.AppendLine("\t\t/// <param name=\"animatorControllers\">The Animator Controllers to add the states to.</param>");
                    fileString.AppendLine("\t\t/// <param name=\"firstPersonAnimatorControllers\">The first person Animator Controllers to add the states to.</param>");
                    fileString.AppendLine("\t\tpublic override void BuildAnimator(AnimatorController[] animatorControllers, AnimatorController[] firstPersonAnimatorControllers)");
                    fileString.AppendLine("\t\t{");
        ...
        private static bool GenerateAnimatorCode(AnimatorController animatorController, string animatorVariableName, string parameterName, float parameterValue, StringBuilder generatedCode)
        {
            ...
            for (int i = 0; i < animatorController.layers.Length; ++i) {
                ...
                if (stateMachines.Count > 0) {
                    if (!generateAnimatorCode) {
                        generatedCode.AppendLine("\t\t\tfor (int i = 0; i < " + animatorVariableName + ".Length; ++i) {");
                    ...
                    for (int j = 0; j < stateMachines.Count; ++j) {
                        ...
                        generatedCode.AppendLine("\t\t\t\tif (" + animatorVariableName + "[i].layers.Length <= " + i + ") {");
                        ...
                        generatedCode.AppendLine("\t\t\t\t}");
                        generatedCode.AppendLine();
                        generatedCode.AppendLine("\t\t\t\tvar " + baseStateMachineName + " = " + animatorVariableName + "[i].layers[" + i + "].stateMachine;");
                        generatedCode.AppendLine();
                        generatedCode.AppendLine("\t\t\t\t// The state machine should start fresh.");
                        generatedCode.AppendLine("\t\t\t\tfor (int j = 0; j < " + animatorVariableName + "[i].layers.Length; ++j) {");
                        generatedCode.AppendLine("\t\t\t\t\tfor (int k = 0; k < " + baseStateMachineName + ".stateMachines.Length; ++k) {");
                        generatedCode.AppendLine("\t\t\t\t\t\tif (" + baseStateMachineName + ".stateMachines[k].stateMachine.name == \"" + baseStateMachine.stateMachine.name + "\") {");
                        generatedCode.AppendLine("\t\t\t\t\t\t\t" + baseStateMachineName + ".RemoveStateMachine(" + baseStateMachineName + ".stateMachines[k].stateMachine);");
            ...
            if (stateMachine.states.Length > 0) {
                ...
                for (int i = 0; i < stateMachine.states.Length; ++i) {
                    ...
                    if (state.motion is BlendTree) {
                        ...
                        GenerateBlendTree(blendTreeName, blendTree, generatedCode, 4);
        ...
        private static void GenerateBlendTree(string blendTreeName, BlendTree blendTree, StringBuilder generatedCode, int indentLevel)
        {
            ...
            generatedCode.AppendLine(indentation + "AssetDatabase.AddObjectToAsset(" + blendTreeName + ", animatorControllers[i]);");
