    ...
    public class UltimateCharacterLocomotionInspector : CharacterLocomotionInspector
    {
        ...
        private void AddAbilityList(VisualElement container, bool addAbilities)
        {
            ...
            }, (int index) => // Remove.
            {
                ...
                // Use the index parameter (the row being removed) rather than SelectedIndex, which can be stale after reordering.
                var ability = (addAbilities ? m_CharacterLocomotion.Abilities : m_CharacterLocomotion.ItemAbilities)[index];
                if (ability != null) {
                    m_LabelByAbilityMap.Remove(ability);
            ...
            }, (int fromIndex, int toIndex) => // Reorder.
            {
                ...
                // Keep selection following the moved item so Remove and EditorPrefs stay correct.
                var selectedIndex = reorderableList.SelectedIndex;
                if (selectedIndex == fromIndex) {
                    reorderableList.SelectedIndex = toIndex;
                } else if (selectedIndex == toIndex) {
                    // List already set selection to toIndex (user dragged the selected item); leave it.
                } else if (fromIndex < toIndex && selectedIndex > fromIndex && selectedIndex <= toIndex) {
                    reorderableList.SelectedIndex = selectedIndex - 1;
                } else if (fromIndex > toIndex && selectedIndex >= toIndex && selectedIndex < fromIndex) {
                    reorderableList.SelectedIndex = selectedIndex + 1;
                }
                EditorPrefs.SetInt((addAbilities ? SelectedAbilityIndexKey : SelectedItemAbilityIndexKey), reorderableList.SelectedIndex);
            });
