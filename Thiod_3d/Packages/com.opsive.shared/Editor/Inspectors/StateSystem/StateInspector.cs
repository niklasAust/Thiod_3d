    ...
    public static class StateInspector
    {
        ...
        public static void OnStateListDraw(object obj, Opsive.Shared.StateSystem.State[] states, SerializedProperty statesProperty, Rect rect, int index)
        {
            ...
            if (GUI.Button(new Rect(startRectX + persistWidth / 2, rect.y + 1, 18, EditorGUIUtility.singleLineHeight), Utility.InspectorStyles.PersistIcon, Utility.InspectorStyles.NoPaddingButtonStyle)) {
                ...
                if (desiredPreset.Data.LongValueHashes != null && desiredPreset.Data.LongValueHashes.Length > 0) {
                    valuePositionMap = new Dictionary<long, int>(desiredPreset.Data.LongValueHashes.Length);
                    for (int i = 0; i < desiredPreset.Data.LongValueHashes.Length; ++i) {
                        valuePositionMap.Add(desiredPreset.Data.LongValueHashes[i], i);
                    }
                } else if (desiredPreset.Data.ValueHashes != null){
                    valuePositionMap = new Dictionary<long, int>(desiredPreset.Data.ValueHashes.Length);
                ...
                var hashType = Serialization.GetHashType(desiredPreset.Data.Version);

                long[] valueHashes;
                if (desiredPreset.Data.LongValueHashes != null && desiredPreset.Data.LongValueHashes.Length > 0) {
                    valueHashes = desiredPreset.Data.LongValueHashes;
                } else {
                    valueHashes = new long[desiredPreset.Data.ValueHashes.Length];
                    for (int i = 0; i < valueHashes.Length; ++i) {
                        valueHashes[i] = desiredPreset.Data.ValueHashes[i];
                    }
                }
                ...
                for (int i = 0; i < properties.Length; ++i) {
                    ...
                    if (valuePositionMap.ContainsKey(hash)) {
                        ...
                        if (!typeof(Object).IsAssignableFrom(property.PropertyType)) {
                            ...
                            Serialization.GetUnityObjectIndexes(ref unityObjectIndexes, property.PropertyType, property.Name, 0, valuePositionMap, valueHashes, desiredPreset.Data.ValuePositions,
                                                                desiredPreset.Data.Values, false, MemberVisibility.Public, hashType);

                            Serialization.RemoveProperty(i, unityObjectIndexes, desiredPreset.Data, MemberVisibility.Public, hashType);
